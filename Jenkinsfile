pipeline {
    ...
    environment {
        APP_NAME = 'products-ms'
        REPO_URL = 'https://github.com/Nest-Microservices-Product/products-ms'
        SSH_CRED_ID = 'ssh-key-ec2'
        SSH_CRED_ID_DIEGO = 'ssh-key-ec2-diego'
        EC2_USER = 'ubuntu'
        REMOTE_PATH = '/home/ubuntu/products-ms'
        K8S_REMOTE_PATH = '/home/ubuntu/nest-microservices/k8s/store-ms/templates/product-ms/'
        IMAGE_NAME = 'fernandoflores07081/products-ms-prod'
    }
        ...
        stage('Deploy to GKE - Production') {
            ...
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/product-ms products-ms-prod=${IMAGE_NAME}:${env.DOCKER_TAG} --namespace=default"

                    // Verificar que el despliegue se complete exitosamente
                    sh "kubectl rollout status deployment/product-ms --namespace=default"
                }
            }
        }
